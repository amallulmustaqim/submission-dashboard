...
function refreshSecModalContent(){
  ...
  DTRC.renderTimeline(DTRC.$('#secTimeline'), secCurrent, false);
  ...
  const dtf = (el,val)=>{ const x=DTRC.$(el); if(x) x.value=DTRC.isoToLocalDT(DTRC.addBizDays(10)); };
  dtf('#secDueAtFR'); dtf('#secDueAtAP'); dtf('#secDueAtNA');
  ...
  if(addInt) addInt.onclick=()=>{
    ...
    DTRC.pushTimeline(arr[idx], {actor:'sec', stage: arr[idx].status, internal:true, text}); 
    if(!DTRC.safeSaveProjects(arr)) return;
    secCurrent=arr[idx]; DTRC.$('#secInternalNote').innerHTML=''; DTRC.renderTimeline(DTRC.$('#secTimeline'), secCurrent, false); renderSec(); DTRC.notify('Internal note added');
  };
}
...
document.addEventListener('DOMContentLoaded', ()=>{
  ...
  if(applyBtn) applyBtn.onclick= async ()=>{
    ...
    if(val==='FEEDBACK_REQUIRED'){
      ...
      const dueSel=DTRC.$('#secDueAtFR').value; p.dueAt = dueSel? DTRC.localDTToISO(dueSel): DTRC.addBizDays(10);
    ...
    if(val==='APPROVED'){
      ...
      const dueSel=DTRC.$('#secDueAtAP').value; p.dueAt = dueSel? DTRC.localDTToISO(dueSel): DTRC.addBizDays(10);
    ...
    if(val==='NOT_APPROVED'){
      ...
      mo.forEach(f=> p.attachments.push({id:'A'+Math.random().toString(36).slice(2,8),kind:'meeting_outcome',filename:f.name,mimeType:f.type,size:f.size,uploadedBy:'secretariat',uploadedAt:new Date().toISOString(),dataUrl:f.dataUrl}));
      const dueSel=DTRC.$('#secDueAtNA').value; p.dueAt = dueSel? DTRC.localDTToISO(dueSel): DTRC.addBizDays(10);
